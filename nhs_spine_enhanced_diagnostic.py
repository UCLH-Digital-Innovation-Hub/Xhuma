            ...
            if response_body.strip().startswith("{"):
                ...
                if fhir_data.get("resourceType") == "OperationOutcome":
                    ...
                    for issue in issues:
                        ...
                        logger.error(
                            f"🚨 FHIR Issue: {issue_detail['severity'].upper()} - {issue_detail['code']}"
                        )
                        logger.error(f"   Details: {issue_detail['details']}")
                        if issue_detail["diagnostics"] != "No diagnostics":
                            logger.error(
                                f"   Diagnostics: {issue_detail['diagnostics']}"
                            )
        ...
        for path in test_paths:
            ...
            try:
                ...
                conn = http.client.HTTPSConnection(
                    self.host, self.port, context=ssl_context, timeout=30
                )
                ...
                if response.status >= 400:
                    error_analysis = self.parse_fhir_error(
                ...
                if response.status == 200:
                    ...
                    response.status == 403
                    and "application/fhir" in response_headers.get("content-type", "")
                ):
                    logger.info(
                        "✅ FHIR 403 response - authentication/authorization issue identified"
                    )
                elif (
                    response.status == 404
                    and "application/fhir" in response_headers.get("content-type", "")
                    ...
                    logger.info(
                        "✅ FHIR 404 response - endpoint exists but resource not found"
                    )
    ...
    def generate_summary_report(self, test_results: Dict[str, Any]) -> str:
        ...
        for i, result in enumerate(test_results.get("results", []), 1):
            if "error" in result:
                ...
                report.append(
                    f"   {i}. {result['path']} - {status} {reason} (Trace: {trace_id})"
                )
