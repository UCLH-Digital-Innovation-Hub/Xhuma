...
async def test_medication_entry_xmltext_validation():
    ...
    med_statement = MedicationStatement(
        {
            ...
            "identifier": [
                {
                    "system": "urn:oid:2.16.840.1.113883.2.1.3.2.4.18.22",
                    "value": "test-uuid",
                }
            ],
            "status": "active",
    ...
    entry_with_row = await medication_entry(
        med_statement, {"Medication/1": mock_med, "MedicationRequest/1": mock_med_req}
    )
    ...
    imm = Immunization(
        {
            ...
            "note": [{"text": "Refused"}, {"text": "Patient was ill"}],
        }
    )
