...
def test_iti38_rejects_invalid_saml_issuer(monkeypatch):
    ...
    response = client.post("/SOAP/iti38", content=invalid_soap_xml, headers={"Content-Type": "application/soap+xml"})
...
def test_iti38_accepts_valid_saml_issuer(monkeypatch):
    ...
    monkeypatch.setattr("app.soap.soap.process_saml_attributes", lambda x: (_ for _ in ()).throw(HTTPException(status_code=200, detail="Success")))
    ...
    response = client.post("/SOAP/iti38", content=valid_soap_xml, headers={"Content-Type": "application/soap+xml"})
...
def test_iti39_rejects_ssrf_reply_to(monkeypatch):
    ...
    response = client.post("/SOAP/iti39", content=ssrf_soap_xml, headers={"Content-Type": "application/soap+xml"})
...
def test_iti39_accepts_valid_reply_to(monkeypatch):
    ...
    response = client.post("/SOAP/iti39", content=valid_soap_xml, headers={"Content-Type": "application/soap+xml"})
